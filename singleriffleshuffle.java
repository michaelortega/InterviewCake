...
public class Solution {
        ...
        int halfTwoPtr = 0;
        int index = 0;
        int shuffedDeckCard = 0;

        while (index < shuffledDeck.length) {
            shuffedDeckCard = shuffledDeck[index++];

            if (halfOnePtr < half1.length  && shuffedDeckCard == half1[halfOnePtr]) {
                halfOnePtr++;
            } else if (halfTwoPtr < half2.length  && shuffedDeckCard == half2[halfTwoPtr]) {
                halfTwoPtr++;
            }else {
                return false;
            }
